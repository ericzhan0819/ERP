...
- 最新驗證狀態：Phase 4D-2B revenue-side draft generation active；convert route uses `AccountingEventConvertService`；reviewed `vehicle_sale_completed` with valid DB-backed AR / Sales Revenue mappings creates one draft journal and exactly two lines；journal remains draft。
