...
Transaction Completion remains non-recognition。完成交易目前會記錄交易完成狀態、建立一筆 pending Accounting Event、寫入 audit event；reviewed event 在 config enabled 且 DB-backed AR / Sales Revenue mapping 存在時，可轉成 draft journal 與兩條 revenue-side lines，但仍不自動過帳、不做 COGS / tax / profit / gross margin。
