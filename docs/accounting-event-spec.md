...
docs/accounting-event-runtime-mapping-decision-spec.md
AccountingEventJournalDraftPreflightService
AccountingEventAccountMappingResolver
AccountingEventAccountMapping model / policy / controller / requests / UI
```
...
- Runtime account IDs are resolved through DB-backed mapping when preflight metadata allows it.
...
Phase 4D-2A-2: Database-backed Mapping Foundation verification / normalization next.
