...
export default function EditAccountingEventMapping({ auth, mapping, supportedEventTypes = {}, mappingKeyOptions = {}, accountOptions = [], branchOptions = [] }) {
    const { data, setData, patch, processing, errors } = useForm({
        event_type: mapping.event_type ?? 'vehicle_sale_completed',
        mapping_key: mapping.mapping_key ?? 'accounts_receivable_account',
    ...
    const submit = (event) => {
        ...
        patch(route('employee-system.accounting.event-mappings.update', mapping.id));
    };

    return (
        <DashboardLayout user={auth.user}>
            <div className="mx-auto max-w-4xl space-y-5 p-4 md:p-6">
                <Header title="編輯會計事件映射" description="更新 preflight 使用的 DB-backed 科目對應。" />
